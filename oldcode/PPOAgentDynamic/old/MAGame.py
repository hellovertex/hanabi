...
class MAPlayer():
    ...
    def ready_to_train(self):
        for i in range(self.nenvs):
            if len(sel.history_buffer[i]['values']) < p.nsteps + 1:
                return False
        return True
    ...
    def get_training_data(self):
        ...
        self.history_buffer = [defaultdict(list) for _ in range(self.nenvs)]
        return obses, actions, probs, neglogps, values, rewards, dones, masks, states, states_v, legal_moves
    
    
class MAGame():
    def __init__(self, num_players, models, load_env, clip_func, train_schedule = 'par', updates_wait = 10):
        # par or turn
    ...
    def reset(self):
        ...
        self.noise_val_list = {p.num : None for p in self.players}
    ...
    def train_model(self, k, target_kl, return_state_probs = True):
        ...
        state_probs = [[] for _ in range(self.nplayers)]
        for p in self.players:
            
            (mb_obs, mb_actions, mb_probs, mb_neglogps, mb_legal_moves, mb_values,
             mb_rewards, mb_masks, mb_dones, mb_states, mb_states_v, mb_noise) = self.collect_data(p)
            action_distributions[p.num] = mb_actions
            state_probs[pnum] = (mb_obs, mb_actions, mb_probs, mb_legal_moves)
            if self.schedule is 'turn':
                if self.train_player != p.num:
                    continue
        ...
        if self.schedule is 'turn':
            if self.train_counter % self.updates_wait == 0:
                self.train_counter = 0
                self.train_player = (self.train_player + 1) % self.num_players
        if return_state_probs:
            return state_probs, policy_losses, value_losses, policy_entropies, train_steps, action_distributions
        else:
            return policy_losses, value_losses, policy_entropies, train_steps, action_distributions
