...
flags.DEFINE_integer('num_parallel_environments', 5,
...
def train_eval(
        ...
        collect_episodes_per_iteration=90,  # lowered from 90 to 30
    ...
    with tf.compat.v2.summary.record_if(lambda: tf.math.equal(global_step % summary_interval, 0)):
        ...
        # ---------------- Create Metrics ---------------- #
        # ################################################ #
        train_metrics, step_metrics, environment_steps_count = get_metrics_train_and_step(num_eval_episodes,
        ...
        with tf.compat.v1.Session(tf_master) as sess:
            ...
            while sess.run(environment_steps_count) < num_environment_steps:
                ...
                sess.run(collect_op)
                arr = sess.run(replay_buffer.gather_all())
                print(arr)
                print(arr[0].shape)
