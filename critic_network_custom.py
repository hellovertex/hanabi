...
class CriticNetwork(network.Network):
  ...
  def __init__(self,
    ...
    # replaced observation_spec with observation_spec['state']
    if len(tf.nest.flatten(observation_spec['state'])) > 1:
  ...
  def call(self, inputs, step_type=(), network_state=()):
    ...
    # replaced observations with observations['state']
    observations = tf.cast(tf.nest.flatten(observations['state'])[0], tf.float32)
