...
class DynamicStepDriver(driver.Driver):
    ...
    Overwrote
    self._loop_condition_fn() method, s.t. it only runs until environment is reset,
    in order to avoid going below 0 life tokens
    """
    ...
    def _loop_condition_fn(self):
        ...
        def loop_cond(counter, *_):
            ...
            """
            return not self.env._env.envs[0].wrapped_env().state.is_terminal()
    ...
    def _loop_body_fn(self):
        ...
        def loop_body(counter, time_step, policy_state):
            ...
            def get_mask_legal_moves(num_moves, observation):
