...
def color_plausible(color, player_card_knowledge, card_id, card_color_revealed, last_hand, last_player_action,
...
def rank_plausible(rank, player_card_knowledge, card_id, card_rank_revealed, last_hand, last_player_action,
                   last_player_card_knowledge):
    plausible = True
    if last_hand:
        if last_player_action == "PLAY" or last_player_action == "DISCARD":
        ...
        for ih, player_card_knowledge in enumerate(card_knowledge_list):
            ...
            for card_id, card in enumerate(player_card_knowledge):
                ...
                for color in range(self.num_colors):
                    ...
                    if color_plausible(color, player_card_knowledge, card_id, card_color_revealed, last_hand,
                                       self.last_player_action["action_type"], self.last_player_card_knowledge):

                        for rank in range(self.num_ranks):

                            if rank_plausible(rank, color, player_card_knowledge, card_id, card_rank_revealed,
                                              last_hand, self.last_player_action["action_type"],
                                              self.last_player_card_knowledge):
                                card_index = color * self.num_ranks + rank
