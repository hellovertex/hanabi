...
class HanabiEnv(Environment):
    ...
    def __init__(self, config, w_hint, w_play, w_disc,
                 augment_flag = False, binary_augment_flag=False):
        ...
        """
        # set weights for custom rewards
        self.w_hint = w_hint
        self.w_play = w_play
        self.w_disc = w_disc

        self.OPEN_HANDS = False
        self.augment_input = augment_flag
        self.augment_input_using_binary = binary_augment_flag
    ...
    def step(self, action, eval_mode):
        """Take one step in the game.
        Args:
        ...
        info = {}  # reward provided by environment as well as custom rewards will be saved separately here to
        # monitor learning
        custom_reward = 0
        ...
        if not eval_mode:
            if action.type() in [REVEAL_COLOR, REVEAL_RANK]:
                #TODO: parameterize playable and last copy hints individually
                hint_reward = np.sum(self.reward_metrics.maybe_change_hint_reward(action, self.state)) * self.w_hint
                custom_reward += hint_reward
            ...
            if action.type() == DISCARD:
                discard_reward = self.reward_metrics.maybe_change_discard_reward(action, self.state) * self.w_disc
                custom_reward += discard_reward  # TODO: discard_reward is not balanced yet
                info["discard_reward"] = discard_reward
                # print(f"disc reward: {discard_reward}")
        ...
        if self.augment_input:
            observation = self.observation_augmenter.augment_observation(observation=observation,
                                                                         ...
                                                                         # prev_player_hands was given by reference
                                                                         # and modified meanwhile
                                                                         action=action)
        ...
        # -------------- state space END --------------- #
        env_reward = self.state.score() - last_score
        info["environment_reward"] = env_reward
        # so for monitoring purposes, we also save the ingame reward (i.e. "non-custom" reward). this is of
        # course redundant, but allows us to sanity check whether custom rewards and the in game reward add up
        # correctly

        if not eval_mode:
            #hamming distance currently not included
            ...
            reward = env_reward + custom_reward
        else:
            reward = env_reward
...
def make(environment_name="Hanabi-Full", num_players=2, w_hint=1, w_play=1, w_disc=1, pyhanabi_path=None):
    """Make an environment.
    Args:
