...
def train_eval(
        ...
        eager_eval_reset_steps=10000
):
    """A simple train and eval for TD3."""
    root_dir = os.path.expanduser(root_dir)
    train_dir = os.path.join(root_dir, 'train')
    eval_dir = os.path.join(root_dir, 'eval')
    for i in range(eager_eval_reset_steps):
        """ Add an additional loop here that restarts training, so that eager evaluation does not require too much 
        memory. We do this at the cost of again setting up the training each iteration"""
        ...
        with tf.compat.v2.summary.record_if(
            ...
            tf_env = tf_py_environment.TFPyEnvironment(load_hanabi_env())
            eval_tf_env = tf_py_environment.TFPyEnvironment(load_hanabi_env())

            actor_net = masked_networks.MaskedActorRnnNetwork(
            ...
            critic_net = masked_networks.MaskedCriticRnnNetwork(
