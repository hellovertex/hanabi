...
from tf_agents_lib import categorical_projection_network_custom
...
class ActorDistributionNetwork(network.DistributionNetwork):
    ...
    # needs to unpack observations?
    def call(self, observations, step_type, network_state):
        del step_type  # unused.

        # replace observations with observations['state'] and pass observations['mask'] seperately
        mask = observations['mask']
        outer_rank = nest_utils.get_outer_rank(observations['state'], self.input_tensor_spec['state'])
        observations = tf.nest.flatten(observations['state'])
        ...
        output_actions = tf.nest.map_structure(
            lambda proj_net: proj_net(states, outer_rank, mask),
