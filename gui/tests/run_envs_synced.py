            ...
            while not done:
                ...
                if it == 0:
                    for i in range(len(agents)):
                        ...
                        # inject init message
                        msg_init = pyhanabi_to_json.create_init_message(observation_tmp, i)
                        self.game_state_wrappers[i].init_players(msg_init)

                        # inject notifyList message
                        msg_notifyList = pyhanabi_to_json.create_notifyList_message(observation_tmp)
                ...
                for i in range(len(agents)):
                    notify_msg = pyhanabi_to_json.create_notify_message_from_last_move(self.game_state_wrappers[i],last_moves, current_player)
                    ...
                    if len(last_moves) > 0 and last_moves[0].move().type() == utils.HanabiMoveType.DEAL:
                        ...
                        deal_msg = pyhanabi_to_json.create_notify_message_deal(self.game_state_wrappers[i],last_moves, current_player)
